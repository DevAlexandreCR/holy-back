...
ALTER TABLE `devotional_notification_sends`
  ADD INDEX `dev_notif_sends_user_type_acc_idx`(`user_id`, `type`, `provider_accepted_at`),
  ADD INDEX `dev_notif_sends_user_type_open_idx`(`user_id`, `type`, `opened_at`);
...
CREATE TABLE `devotional_notification_evaluation_daily_metrics` (
  ...
  UNIQUE INDEX `dev_notif_eval_daily_date_type_key`(`date`, `notification_type`),
  INDEX `dev_notif_eval_daily_type_date_idx`(`notification_type`, `date`),
  PRIMARY KEY (`id`)
